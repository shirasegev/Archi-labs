...
void relocationTables(){
    ...
    int i;
    for (i = 0; i < ehdr->e_shnum; i++){
        if (strcmp(headerStrTablePtr + sectionHeaders[i].sh_name, ".dynstr") == 0)
            strTable = addr + sectionHeaders[i].sh_offset;
        if (sectionHeaders[i].sh_type == SHT_DYNSYM){
            dynsymTable = addr + sectionHeaders[i].sh_offset;
        }
    }

    for (i = 0; i < ehdr->e_shnum; i++){
        if (sectionHeaders[i].sh_type == SHT_REL) {
            relocationTable = addr + sectionHeaders[i].sh_offset;
            int symSize = sectionHeaders[i].sh_size / sizeof(Elf32_Rel);
            printf("\nRelocation section '%s' at offset %0#x contains %d entries:\n",
            headerStrTablePtr + sectionHeaders[i].sh_name, sectionHeaders[i].sh_offset, symSize);
            printf("Offset\t\tInfo\t\tType\tSym.Value\tSym.Name\n");
            
            int j;
            for (j = 0; j < symSize; j++){
                int index = ELF32_R_SYM(relocationTable[j].r_info);
                Elf32_Sym *entry = dynsymTable + index;
                printf("%08x\t%08x\t%d\t%08x\t%s\n",
                relocationTable[j].r_offset, relocationTable[j].r_info,
                ELF32_R_TYPE(relocationTable[j].r_info), entry->st_value,
                strTable + entry->st_name);
            }
        }
    }
}
