#include "util.h"
extern int system_call(int, ...);

#define SYS_OPEN 5
#define SYS_CLOSE 6
#define SYS_READ 3
#define SYS_WRITE 4

#define SYS_EXIT 1
#define ERROR_CODE 0x55

#define STDIN 0
#define STDOUT 1
#define STDERR 2

#define O_RDONLY 0
#define O_WRONLY 1
#define O_CREAT 64

int call_system_call(int call_id, void* param1, void * param2, void* param3, int debug) {
  int result = system_call(call_id, param1, param2, param3);
  if (debug) {
    char *r = itoa(call_id);
    system_call(SYS_WRITE, STDERR, " - call id: ", 12);
    system_call(SYS_WRITE, STDERR, r, strlen(r));
    system_call(SYS_WRITE, STDERR, ", return code: ", 15);
    r = itoa(result);
    system_call(SYS_WRITE, STDERR, r, strlen(r));
    system_call(SYS_WRITE, STDERR, "\n", 1);
  }
  return result;
}

int main(int argc, char **argv) {

  char c = 0, d = 0;
  int debug = 0;
  int encrypt = 0;
  char *key = 0;
  int index = 0;
  int keyLen = 0;

  int input = STDIN;
  int output = STDOUT;
  char *input_name = "stdin";
  char *output_name = "stdout";

  int result;
  int i;

  for (i = 1; i < argc; i++){
    if(strcmp(argv[i],"-D") == 0){
      debug = 1;
    }
    else if (strncmp(argv[i], "-i", 2) == 0){
      input = system_call(SYS_OPEN, argv[i]+2, O_RDONLY, 0777);
      input_name = argv[i]+2;
    }
    else if (strncmp(argv[i], "-o", 2) == 0){
      output = system_call(SYS_OPEN, argv[i]+2, O_CREAT | O_WRONLY, 0777);
      output_name = argv[i]+2;
    }
    else if (strncmp(argv[i], "-e", 2) == 0){
      encrypt = -1;
      key = argv[i]+2;
      keyLen = strlen(key);
    }
    else if (strncmp(argv[i], "+e", 2) == 0){
      encrypt = 1;
      key = argv[i]+2;
      keyLen = strlen(key);
    }
    else {
      system_call(SYS_WRITE, STDOUT, "invalid parameter - ", 20);
      system_call(SYS_WRITE, STDOUT, argv[i], strlen(argv[i]));
      system_call(SYS_WRITE, STDOUT, "\n", 1);
	    return 1;
    }
  }

  if (debug) {
    for (i = 1; i < argc; i++){
      system_call(SYS_WRITE, STDERR, argv[i], strlen(argv[i]));
      system_call(SYS_WRITE, STDERR, "\n", 1);
    }
    system_call(SYS_WRITE, STDERR, "Input file name: ", 17);
    system_call(SYS_WRITE, STDERR, input_name, strlen(input_name));
    system_call(SYS_WRITE, STDERR, "\n", 1);
    system_call(SYS_WRITE, STDERR, "Output file name: ", 18);
    system_call(SYS_WRITE, STDERR, output_name, strlen(output_name));
    system_call(SYS_WRITE, STDERR, "\n", 1);
  }
  
  while((result = call_system_call(SYS_READ, (void*)input, &c, (void*)1, debug)) > 0){

    if (encrypt == 0) {
      if (c >= 'a' && c <= 'z'){
        d = c - 'a' + 'A';
      }
      else {
        d = c;
      }
    }
    else {
      if (c == '\n'){
        d = c;
        index = 0;
      }
      else {
        d = c + encrypt * (key[index] - '0');
        index = (index + 1) % keyLen;
      }
    }
    system_call(SYS_WRITE, output, &d, 1);
  }

  if (input != STDIN){
    result = call_system_call(SYS_CLOSE, (void*)input, 0, 0, debug);
    if (result < 0) system_call(SYS_EXIT, ERROR_CODE);
  }
  if (output != STDOUT){
    result = call_system_call(SYS_CLOSE, (void*)output, 0, 0, debug);
    if (result < 0) system_call(SYS_EXIT, ERROR_CODE);
  }
  return 0;
}